...
# Force-reseed config every boot so openclaw.json changes in the image
# always take effect (doctor --fix was overwriting the config and breaking
# plugin settings like channels.slack).
cp "$DEFAULT_CONFIG" "$CONFIG_FILE"
